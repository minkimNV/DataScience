...
for i in range(n-1, 0, -1):                 # 오름차순 정렬이니까 맨 뒤에서부터 for-loop.
    ...
    if arr[idx] > arr[i]:                   # 그 인덱스가 가진 값이 arr[i]보다 크면
        ...
        if k_count == k:                    # 실제 교환 횟수가 입력 교환 횟수보다 크던 말던 입력받은 교환 횟수만큼 돌았으면
            for num in arr:
                print(num, end=' ')         # 배열 원소 한 줄에 뽑아줘용
            break
        # else: print(-1)
        # 원래 처움에 break 없이 *그렇지 않으면 -1 출력*을 했는데 그럼 if절 돌리는 내내 k_count가 k보다 커질때까지 -1 출력하더라? 그래서 내쫒아버림
if k_count < k:
...
#             exit()
# print(-1)
# 근데 이렇게 넣으면 안대.. 그냥 하면 끝까지 다 도는데 왜 구래..?ㅠ 속상해짜나..
